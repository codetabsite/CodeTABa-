...
class ApiKeyDialog(QDialog):
    ...
    def _build_ui(self, first_run: bool):
        ...
        if first_run:
            info = QLabel(
                "İlk çalıştırma — Gemini API key gerekiyor.\n"
                "Ücretsiz key almak için: <a href='https://aistudio.google.com/apikey' "
                "style='color:#58a6ff'>aistudio.google.com/apikey</a>"
            )
        else:
    ...
    def _save(self):
        ...
        if not key.startswith("AIza") or len(key) < 20:
            QMessageBox.warning(
                self, "Geçersiz Key",
                "API key 'AIza' ile başlamalı ve geçerli olmalıdır."
            )
            return
        save_api_key(key)
        self.accept()
